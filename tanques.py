...
class Quatro_Tanques:
    def __init__(self, dt=4, render_mode=None, setpoint=[6.2,6.35], max_steps=500, tol=1e-3):
    ...
    def step(self, actions_norm = [0,0], u=None):
        ...
        u_phys = u_phys if (u == None) else u
        ...
        if abs(error1) < self.tol:
            reward1 += 1000 # Bônus extra por atingir o alvo
        
        if abs(error2) < self.tol:
            reward2 += 1000 # Bônus extra por atingir o alvo
