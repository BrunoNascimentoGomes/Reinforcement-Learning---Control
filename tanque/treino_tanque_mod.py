...
for episode in range(MAX_EPISODES):
    ...
    for step in range(MAX_STEPS):
        ...
        done_sig = np.logical_or(terminated, truncated).astype(np.float32)
        done_sig = done_sig.reshape(NUM_AGENTS, 1)
        #print(done_sig)
    ...
    if total_reward > best_reward:
        maddpg.save('./maddpg_tito_parcial')
        best_reward = total_reward
        print(f'Modelo Salvo com recompensa: {best_reward:.2f}')
