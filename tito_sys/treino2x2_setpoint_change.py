...
NUM_AGENTS = 2
STATE_DIM = 7     # [erro, integral erro,y,dy, u, delta_u, setpoint]
ACTION_DIM = 1     # Controle u1 ou u2
MAX_ACTION = 1.0   

# Parâmetros de Exploração (Ruído)
NOISE_START = 0.5          # Começa alto (3V físicos) para explorar bem
NOISE_END = 0.01           # Termina baixo para ajuste fino
DECAY_RATE = 0.9922        # Decaimento suave para durar todo o treino
...
BATCH_SIZE = 256
MAX_EPISODES = 2000       # Quantidade de episódios
MAX_STEPS = 2000            # Passos por episódio (tempo suficiente para estabilizar)
...
# -----------------------------------------------------
setpoint_schedule = [
    (0,   [1.0, 1.0]),  # Começa baixo
    (200, [6.0, 1.0]),  # Sobe apenas Y1
    (500, [3.0, 7.0]),
    (800, [2.0, 1.0]),# Sobe Y2   # Desce ambos
]
for episode in range(MAX_EPISODES):
    ...
    for step in range(MAX_STEPS):
        ...
        if sp_index < len(setpoint_schedule) and step >= setpoint_schedule[sp_index][0]:
            ...
            new_sp = setpoint_schedule[sp_index][1]
            env.setpoint = new_sp
            sp_index += 1


            # 1. Recuperamos o y físico atual direto da memória do ambiente
            # env.state = [y1, dy1, y2, dy2]
            current_y1 = env.state[0]
            current_y2 = env.state[2]
            
            # 2. Recalculamos o erro com base no NOVO setpoint
            new_err1 = new_sp[0] - current_y1
            new_err2 = new_sp[1] - current_y2
            
            # 3. Injetamos os novos valores no vetor obs atual

            
            # --- Agente 1 ---
            # Atualiza Erro (idx 0) e Setpoint (idx 4)
            obs[0][0] = env._normalize(new_err1, 'e')
            obs[0][6] = env._normalize(new_sp[0], 'sp')
            
            # --- Agente 2 ---
            obs[1][0] = env._normalize(new_err2, 'e')
            obs[1][6] = env._normalize(new_sp[1], 'sp')
