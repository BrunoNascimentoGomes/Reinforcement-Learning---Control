...
class TITOSystem:
    ...
    def step(self, u):
        ...
        #sol = solve_ivp(self._ode, [self.t, self.t + self.dt], self.state, t_eval=[self.t + self.dt], args=(u,), method='RK23')
        sol = solve_ivp(
            self._ode,
            [self.t, self.t + self.dt],
            self.state,
            args=(u,),
            method='RK23',    # mais rápido
            t_eval=[self.t, self.t + self.dt],
            rtol=1e-4,
            atol=1e-6
)
        ...
        error1 = float(self.setpoint[0] - y1)
        error2 = float(self.setpoint[1] - y2)

        self.num_hist.append(self.num_step)
        self.y1_hist.append(y1)
        self.y2_hist.append(y2)
        self.u1_hist.append(u[0])
        self.u2_hist.append(u[1])
        self.setpoint1_hist.append(self.setpoint[0])
        self.setpoint2_hist.append(self.setpoint[1])

        #Obs e Reward
        obs1 = np.array([error1, y1, self.u1_hist[-1], self.setpoint[0]], dtype=np.float32)
        obs2 = np.array([error2, y2, self.u2_hist[-1], self.setpoint[1]], dtype=np.float32)

        obs = np.array([obs1, obs2])
        
        #reward1 = - abs(float(error1)) if abs(error1) > self.tol else 10
        #reward2 = - abs(float(error2)) if abs(error2) > self.tol else 10
        reward1 = - (error1**2) if abs(error1) > self.tol else 10
        reward2 = - (error2**2) if abs(error2) > self.tol else 10
