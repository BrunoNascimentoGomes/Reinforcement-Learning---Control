...
for ep in range(n_episodes):
    ...
    ax.plot(total_reward_plot, range(ep))                  # plota a lista atualizada
    ...
    if ep % 20 == 0:
        mean, std = evaluate_policy(env, all_agents, device, max_action)
        print(f"Ep {ep+1}: Recompensa Média: {mean:.2f} ± {std:.2f}")
        print(f"Ep {ep+1}: noise_std={noise_std:.4f}")


    # critério de salvar baseado em recompensa por agente (exemplo)
    if total_reward1 > -10 and total_reward2 > -10:
        try:
            env.render()
