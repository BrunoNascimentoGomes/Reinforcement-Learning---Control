...
NUM_AGENTS = 2
STATE_DIM = 6     # [erro, integral erro,y, u, delta_u, setpoint]
...
DECAY_RATE = 0.9992        # Decaimento suave para durar todo o treino
...
for episode in range(MAX_EPISODES):
    ...
    steps_until_change = np.random.randint(300, 400) # Muda a cada 300-400 passos
    ...
    for step in range(MAX_STEPS):
        ...
        if (step - last_change_step) > steps_until_change:
            ...
            obs[0][4] = env._normalize(new_sp[0], 'sp')
            
            # --- Agente 2 ---
            obs[1][0] = env._normalize(new_err2, 'e')
            obs[1][4] = env._normalize(new_sp[1], 'sp')
        ...
        reward1 += reward[0]
        reward2 += reward[1]
        reward_tot = reward1 + reward2
        reward = np.array([reward_tot, reward_tot])
